  ...
  >
    <template #item="{ item, root, active, props, hasSubmenu }">
      ...
      <InertiaLink
        ...
        :class="[{ 'font-bold! text-muted-color': item.active }]"
        :style="item.style"
      ...
      <a
        ...
        :class="[hasSubmenu ? 'p-panelmenu-header-link' : 'p-panelmenu-item-link']"
        :style="item.style"
        :aria-disabled="item.disabled === true"
      >
        <i
        ...
        <template v-if="hasSubmenu">
          <ChevronDown v-if="active" class="p-panelmenu-submenu-icon ml-auto" />
          <ChevronRight v-else class="p-panelmenu-submenu-icon ml-auto" />
        </template>
      </a>
    </template>
  </PanelMenu>
